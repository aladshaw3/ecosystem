...
''' Data Clustering Example script:
    ----------------
    ...
    Example looks at determining likely hood of cardiac patients dying based on
    there own features: number of heart attacks, age, etc.
...
def printClustering(clustering):
    ...
    for c in clustering:
        ...
        for p in c.members():
            ...
            if p.getLabel() == 1:
                numPos += 1
        fracPos = float(numPos)/float(numPts)
...
for k in (2,):
    ...
    posFracs = testClustering(patients, k, 0)

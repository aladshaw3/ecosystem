/*!
 *  \file skua_opt.h skua_opt.cpp
 *	\brief Optimization Routine for the SKUA Model
 *	\details This file contains structures and functions associated with performing non-linear
 *			least-squares optimization of the SKUA simulation results against actual kinetic
...
/** C-style object holding data and pointers necessary for running a SKUA optimization. It contains information 
	about the type of optimization requested, the current status of the optimization, the data being compared
	against, and the SKUA_DATA object for the evaluation of a SKUA simulation. The pointers in the structure
	are for the two output files produced by the routine: (i) parameter results and (ii) model comparison
	results.  */
typedef struct
{
...
/** This function will run the SKUA simulation at a given value of surface diffusivity and produce residuals that
...
/** This is the function that is callable by the UI. The user must provide 5 input files to the routine in order to
	...
	standard SKUA simulations (see skua.h).
...
/** \note Much of the structure of these input files are "similar" to that of the input files used in SKUA_SCENARIOS
	(see skua.h), but with some notable differences. Below gives the format for each input file with an example. Make
